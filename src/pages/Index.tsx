...
interface ClientRecord {
  ...
  telegram?: string; whatsapp?: string; birth_date?: string;
  nationality?: string; passport_series?: string; passport_number?: string;
  notes?: string;
}

const EMPTY_CLIENT = {
  name: "", email: "", phone: "",
  telegram: "", whatsapp: "", birth_date: "",
  nationality: "RU", passport_series: "", passport_number: "", notes: "",
};
...
function ManagerClients({ token }: { token?: string }) {
  ...
  const handleCreate = async () => {
    if (!form.name.trim()) { setMsg({ type: "err", text: "Введите имя клиента" }); return; }
    if (!form.email.trim()) { setMsg({ type: "err", text: "Введите email клиента" }); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) {
      setMsg({ type: "err", text: "Некорректный формат email" }); return;
    }
    setSaving(true); setMsg(null);
    try {
      const res = await api({ _action: "create-client", ...form });
      const data = await res.json();
      if (!res.ok) { setMsg({ type: "err", text: data.error || "Ошибка создания" }); return; }
      setMsg({ type: "ok", text: `Клиент ${form.name} успешно создан` });
  ...
  const handleUpdate = async () => {
    ...
    if (!form.name.trim()) { setMsg({ type: "err", text: "Введите имя клиента" }); return; }
    if (!form.email.trim()) { setMsg({ type: "err", text: "Введите email" }); return; }
    setSaving(true); setMsg(null);
    try {
      const res = await api({ _action: "update-client", id: editingClient.id, ...form });
      const data = await res.json();
      if (!res.ok) { setMsg({ type: "err", text: data.error || "Ошибка сохранения" }); return; }
      setMsg({ type: "ok", text: "Данные клиента сохранены" });
  ...
  const handleDelete = async (c: ClientRecord) => {
    ...
    try {
      ...
      setConfirmDelete(null); setEditingClient(null); load();
  ...
  const openEdit = (c: ClientRecord) => {
    ...
    setForm({
      ...
      birth_date: c.birth_date || "", nationality: c.nationality || "RU",
      passport_series: c.passport_series || "", passport_number: c.passport_number || "",
      notes: c.notes || "",
    });
    setMsg(null);
  };
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        <button onClick={() => { setEditingClient(null); setMsg(null); }}
          className="w-9 h-9 rounded-xl bg-[hsl(199,60%,88%)] flex items-center justify-center hover:bg-[hsl(199,50%,80%)] transition-colors">
          <Icon name="ArrowLeft" size={17} className="text-[hsl(213,70%,28%)]" />
        </button>
        <div>
          <h2 className="font-display text-2xl font-semibold text-[hsl(213,80%,15%)]">{editingClient.name}</h2>
          <p className="text-sm text-muted-foreground">
            {editingClient.booking_count} {editingClient.booking_count === 1 ? "бронирование" : editingClient.booking_count < 5 ? "бронирования" : "бронирований"}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <button onClick={() => { setEditingClient(null); setMsg(null); }}
          className="flex-1 border-2 border-[hsl(213,70%,28%)] text-[hsl(213,70%,28%)] py-3 rounded-2xl font-semibold text-sm hover:bg-blue-50 transition-colors">
          Отмена
        </button>
      ...
      <div className="ocean-card rounded-2xl p-5 border-red-200 bg-red-50/50 mb-6">
        ...
        {confirmDelete?.id === editingClient.id ? (
          ...
          <button onClick={() => setConfirmDelete(editingClient)}
            className="w-full border border-red-300 text-red-600 py-2.5 rounded-xl text-sm font-medium hover:bg-red-100 transition-colors flex items-center justify-center gap-2">
  ...
  if (showCreate) return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        <button onClick={() => { setShowCreate(false); setForm(EMPTY_CLIENT); setMsg(null); }}
          className="w-9 h-9 rounded-xl bg-[hsl(199,60%,88%)] flex items-center justify-center hover:bg-[hsl(199,50%,80%)] transition-colors">
          <Icon name="ArrowLeft" size={17} className="text-[hsl(213,70%,28%)]" />
        </button>
        <div>
          <h2 className="font-display text-2xl font-semibold text-[hsl(213,80%,15%)]">Новый клиент</h2>
